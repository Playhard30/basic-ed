...
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $pdf->Cell(67, 3, $row['last_sch'], 'B', 0, 'L');
...
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $get_enrolled_sub = mysqli_query($conn, "SELECT *, COUNT(ensub.enrolled_sub_id), CONCAT(teach.teacher_fname, ' ', LEFT(teach.teacher_mname,1), '. ', teach.teacher_lname) AS fullname FROM tbl_enrolled_subjects AS ensub
    ...
    $pdf->Cell(37,3.5,$row2['subject_code'],0,0,'C');
    ...
    $pdf->Cell(13,3.5,$row2['day'],0,0,'C');
    ...
    $pdf->Cell(22,3.5,$row2['time'],0,0,'C');
    ...
    $pdf->Cell(30,3.5,$row2['fullname'],0,0,'C');
    $pdf->SetFont('Arial','','9');
...
LEFT JOIN tbl_acadyears AS ay ON ay.ay_id = sy.ay_id
LEFT JOIN tbl_semesters AS sem ON sem.semester_id = sy.semester_id
LEFT JOIN tbl_principals AS prin ON prin.prin_id = prin.prin_id
WHERE sy.student_id = $stud_id AND sy.ay_id = '$ay_id' AND sy.semester_id = '$sem_id'") or die(mysqli_error($conn));
while ($row = mysqli_fetch_array($get_stud)) {
    ...
    $pdf->Cell(35, 5, $row['student_lname'], 0, 0, 'C');
    $pdf->Cell(35, 5, $row['student_fname'], 0, 0, 'C');
    $pdf->Cell(35, 5, $row['student_mname'], 0, 1, 'C');
    ...
    $pdf->Cell(135, 3, $row['nationality'], 0, 1, 'C');
    ...
    $pdf->Cell(115, 5, $row['fname'], 0, 0, 'L');
    $pdf->Cell(30, 5, $row['fcontact'], 0, 0, 'C');
    $pdf->Cell(0, 5, '', 'B', 1, 'C');
    ...
    $pdf->Cell(.1, 12, "", 'R', 0);
    $pdf->Cell(50, 6, "TEL/CELLPHONE NO.", 'T', 1);
    $pdf->Cell(115, 5, $row['mname'], 0, 0, 'L');
    $pdf->Cell(30, 5, $row['mcontact'], 0, 1, 'C');
